...
export default function JobDetailPage({
  ...
  const fetchJobDetails = async () => {
    try {
      ...
      const [jobResponse, candidatesResponse] = await Promise.all([
        fetch(`${apiUrl}/projects/${projectId}/jobs/${jobId}`),
        fetch(`${apiUrl}/projects/${projectId}/jobs/${jobId}/candidates`)
      ]);
      ...
      if (!jobResponse.ok) {
        throw new Error(jobData.message || "Could not load job.");
      }
      ...
      setJob(null);
      setCandidates([]);
      setPageMessage(error.message || "Could not load job.");
    } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!uploadedBy.trim()) {
      setPageMessage("Please enter your name or email.");
      return;
    }
    ...
    try {
      ...
      setPageMessage(`${data.message}: ${data.fileName}`);
      setParsedResume({
        name: data.name || "",
        phone: data.phone || "",
      ...
      setPageMessage(error.message || "Upload failed.");
    } finally {
  ...
  const handleCandidateUpdate = async (candidate) => {
    try {
      ...
      setPageMessage("Candidate updated.");
    } catch (error) {
      setPageMessage(error.message || "Could not update candidate.");
    } finally {
  ...
  const handleSingleWhatsApp = async (candidate, link) => {
    try {
      ...
      setPageMessage(error.message || "Could not update last contacted date.");
    }
  };
    ...
    <div className="container">
      <section className="card upload-card">
        ...
        {isLoadingJob ? (
          ...
          <>
            ...
            <p><strong>Min Experience:</strong> {job.min_experience ?? 0}</p>
            <p><strong>Max Experience:</strong> {job.max_experience ?? 0}</p>
      ...
      <section className="card dashboard-card">
        <div className="dashboard-header">
          <h2>Candidates</h2>
          <p>{isLoadingJob ? "Loading candidates..." : `${candidates.length} result(s)`}</p>
        </div>
              ...
              {candidates.length ? (
                candidates.map((candidate) => {
                  ...
                  return (
                    <tr
                      key={candidate._id}
                      className={isContactStale(candidate.lastContacted) ? "stale-row" : ""}
                    >
